## R routines for the package nfd
...
plot.with.ggplot <- function(args) {
    tempdf <- data.frame(Freq=c(args$f1, args$f2),
                         Rank=c(seq(args$f1_length), seq(args$f2_length)),
                         Text=as.factor(c(rep(label1, args$f1_length), rep(label2, args$f2_length)))
                         )
    ...
    freqDiff.plot <- qplot(log(seq(args$max_value)),
                           args$freqDiff,
                           geom="segment",
                           yend=0,
                           xend=log(seq(args$max_value)),
                           size = I(3)
                           ) +
        theme_bw() +
...
plot.nfd <- function(x, y, ..., label1=NULL, label2=NULL, legend_pos=c(.75, .7))
{
    ## plot method for a "nfd" class
    if (is.null(label1)) label1 <- "A"
    if (is.null(label2)) label2 <- "B"
    if (require('ggplot2')) {
        ## ggplot was used in the original paper
        argList <- as.list(match.call(expand.dots=TRUE))
        plot.with.ggplot(argList)
    } else {
        warning('ggplot2 not found. Plotting with defaults.')
    }
